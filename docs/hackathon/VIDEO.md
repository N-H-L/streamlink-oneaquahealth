...
| 3:40–4:10 | About page: the validation panel; then the Singapore tab. | "Every record type is checked by the official HL7 validator against OneAquaHealth's guide: zero errors, and deliberately broken records are rejected. We rebuilt OneAquaHealth's map context from OpenStreetMap so it works in any city, and tested it against their lab data with a pre-specified design, leaving one city out at a time. One signal survived: within a city, streams closer to a wastewater plant do rank higher for lab-measured risk (Spearman 0.22, permutation p = 0.03). Comparing cities is not supported, and the three-feature model we started with did worse than chance on a held-out city, so we did not ship it. The score says where to look first; citizens and the lab do the rest. And adding a city is a config file, so it runs where there is no lab data at all." |
