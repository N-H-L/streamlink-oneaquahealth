...
| 3:40–4:10 | About page: the validation panel; then the Singapore tab. | "Every record type is checked by the official HL7 validator against OneAquaHealth's guide: zero errors, and deliberately broken records are rejected. The map score was tested the same way, holding out whole cities. One signal survived: within a city, streams closer to a wastewater plant do rank higher for lab-measured risk. Comparing cities did not hold up, so we do not claim it. The map says where to look first; citizens and the lab do the rest. And adding a city is one config file, so it runs where there is no lab data at all." |
...
- The full wording of the model result is in README/SUBMISSION; on camera use the short version above and let the About page show the numbers.
