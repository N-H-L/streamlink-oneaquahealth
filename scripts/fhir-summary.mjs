...
const rel = (p) => {
  const n = norm(p);
  for (const k of ["fhir/fsh-generated/", "fhir/negative-tests/", "out/bundles/", "out/resources/"]) {
...
const kindOf = (f) =>
  f.startsWith("fhir/negative-tests/") ? "negative"
  : f.startsWith("out/bundles/") ? "bundle"
  : f.startsWith("out/resources/") ? "stored"
  : "ig";
...
- Files: ${count("ig")} IG resources (profiles, extensions, terminology, examples), ${count("bundle")} transaction bundles produced by the app's own engine (\`out/bundles\`), ${count("stored")} resources as stored after a full lifecycle (\`out/resources\`), ${count("negative")} negative tests (expected to fail)
...
console.log(`Stored:       ${count("stored")} files, ${tot("stored", "errors")} errors, ${tot("stored", "warnings")} warnings`);
